...
fn jobs_command(action: JobsAction) -> Result<()> {
    match action {
        JobsAction::List { config } => {
            ...
            for slug in slugs {
                ...
                let recent = db::query::jobs::list_job_runs(&conn, Some(slug), None, 5, 0)
                    .unwrap_or_default();

                let status_summary = if recent.is_empty() {
                    "none".to_string()
                } else {
                    let completed = recent.iter().filter(|r| r.status == core::job::JobStatus::Completed).count();
                    let failed = recent.iter().filter(|r| r.status == core::job::JobStatus::Failed).count();
                    let pending = recent.iter().filter(|r| r.status == core::job::JobStatus::Pending).count();
                    let running = recent.iter().filter(|r| r.status == core::job::JobStatus::Running).count();
        ...
        JobsAction::Trigger { config, slug, data } => {
            let config_dir = config.canonicalize().unwrap_or(config);
            let cfg = config::CrapConfig::load(&config_dir)?;
            let registry = hooks::init_lua(&config_dir, &cfg)?;
            let pool = db::pool::create_pool(&config_dir, &cfg)?;
            ...
            let conn = pool.get().context("Failed to get DB connection")?;
            let job_run = db::query::jobs::insert_job(
                &conn,
                &slug,
                data_json,
                "cli",
        ...
        JobsAction::Status { config, id, slug, limit } => {
            let config_dir = config.canonicalize().unwrap_or(config);
            let cfg = config::CrapConfig::load(&config_dir)?;
            let registry = hooks::init_lua(&config_dir, &cfg)?;
            let pool = db::pool::create_pool(&config_dir, &cfg)?;
            ...
            if let Some(run_id) = id {
                // Show single run
                let run = db::query::jobs::get_job_run(&conn, &run_id)?
                ...
                // List runs
                let runs = db::query::jobs::list_job_runs(&conn, slug.as_deref(), None, limit, 0)?;
                if runs.is_empty() {
        ...
        JobsAction::Purge { config, older_than } => {
            let config_dir = config.canonicalize().unwrap_or(config);
            let cfg = config::CrapConfig::load(&config_dir)?;
            let pool = db::pool::create_pool(&config_dir, &cfg)?;
            ...
            let conn = pool.get().context("Failed to get DB connection")?;
            let deleted = db::query::jobs::purge_old_jobs(&conn, secs)?;
...
async fn serve_command(config_dir: &Path) -> Result<()> {
    ...
    let scheduler_handle = async move {
        scheduler::start(scheduler_pool, scheduler_runner, scheduler_registry, scheduler_config)
            .await
    };

...
fn register_fields(lua: &Lua, crap: &Table) -> Result<()> {
...
mod tests {
    ...
    fn test_fields_factory_sets_type() {
        ...
        // text factory should set type = "text" and preserve other keys
    ...
    fn test_fields_factory_compatible_with_parse() {
        ...
        // Build a select field via factory
        ...
        // Put it in a fields array and parse
        let fields_arr = lua.create_table().unwrap();
        fields_arr.set(1, result).unwrap();
        let parsed = parse::parse_fields(&fields_arr).unwrap();

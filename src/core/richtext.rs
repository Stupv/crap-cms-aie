...
mod tests {
    ...
    fn render_custom_node_with_callback() {
        let json = r#"{"type":"doc","content":[{"type":"cta","attrs":{"text":"Click me","url":"/go"}}]}"#;
    ...
    fn render_custom_node_passthrough() {
        let json =
            r#"{"type":"doc","content":[{"type":"unknown_widget","attrs":{"foo":"bar"}}]}"#;
        let result = render_prosemirror_to_html(json, &no_custom).unwrap();
    ...
    fn render_html_escape() {
        let json =
            r#"{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"<script>alert('xss')</script>"}]}]}"#;
        let result = render_prosemirror_to_html(json, &no_custom).unwrap();

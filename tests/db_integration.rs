...
fn hydrate_document_populates_join_data() {
    ...
    query::hydrate_document(&conn, "articles", &def, &mut doc, None, None)
...
fn save_join_table_data_from_hashmap() {
    ...
    query::save_join_table_data(&tx, "articles", &def, &doc.id, &jt_data, None)
...
fn save_join_table_data_partial_update() {
    ...
    query::save_join_table_data(&tx, "articles", &def, &doc.id, &jt_data2, None)
        .expect("Save failed");
    tx.commit().expect("Commit");
...
fn populate_depth_1_hydrates_has_many() {
    ...
    let conn = pool.get().expect("DB connection");
    query::hydrate_document(&conn, "posts_v2", &posts_def, &mut post, None, None)
        .expect("Hydrate failed");
...
fn hydrate_returns_grouped_fields() {
    ...
    query::hydrate_document(&conn, "pages_with_seo", &def, &mut doc, None, None)
        .expect("Hydrate");
    ...
    CollectionDefinition {
        ...
        fields: vec![
            ...
            FieldDefinition {
                ...
                ..make_field("meta", FieldType::Blocks)
            },
        ],
        admin: CollectionAdmin::default(),
        hooks: CollectionHooks::default(),
...
fn save_join_table_data_with_locale_scopes_writes() {
    ...
    query::save_join_table_data(&tx, "l10n_articles", &def, &doc.id, &en_join, Some(&en_ctx)).unwrap();
    ...
    query::save_join_table_data(&tx, "l10n_articles", &def, &doc.id, &de_join, Some(&de_ctx)).unwrap();
    tx.commit().expect("Commit");
...
fn hydrate_document_with_locale_returns_correct_data() {
    ...
    let mut en_doc = query::find_by_id(&conn, "l10n_articles", &def, &doc.id, None)
        .unwrap().unwrap();
    query::hydrate_document(&conn, "l10n_articles", &def, &mut en_doc, None, Some(&en_ctx)).unwrap();
    ...
    let mut de_doc = query::find_by_id(&conn, "l10n_articles", &def, &doc.id, None)
        .unwrap().unwrap();
    query::hydrate_document(&conn, "l10n_articles", &def, &mut de_doc, None, Some(&de_ctx)).unwrap();
...
fn non_localized_join_field_ignores_locale_context() {
    ...
    query::save_join_table_data(&tx, "l10n_articles", &def, &doc.id, &join, Some(&en_ctx)).unwrap();
    tx.commit().expect("Commit");
    ...
    let mut doc2 = query::find_by_id(&conn, "l10n_articles", &def, &doc.id, None)
        .unwrap().unwrap();
    query::hydrate_document(&conn, "l10n_articles", &def, &mut doc2, None, Some(&de_ctx)).unwrap();
...
fn hydrate_without_locale_returns_all_locale_rows() {
    ...
    let mut doc2 = query::find_by_id(&conn, "l10n_articles", &def, &doc.id, None)
        .unwrap().unwrap();
    query::hydrate_document(&conn, "l10n_articles", &def, &mut doc2, None, None).unwrap();

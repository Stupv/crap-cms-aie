...
fn global_migration_creates_join_tables() {
    ...
    assert!(
        check("_global_homepage_links"),
        "Array join table should exist"
    );
    assert!(
        check("_global_homepage_content"),
        "Blocks join table should exist"
    );
    assert!(
        check("_global_homepage_featured_posts"),
        "Has-many junction table should exist"
    );
}
...
fn global_migration_parent_table_columns() {
    ...
    assert!(
        ...
        "Should have group sub-field column"
    );
    assert!(
        columns.contains("seo__meta_description"),
        "Should have group sub-field column"
    );
    assert!(
    ...
    assert!(
        !columns.contains("links"),
        "Array field should NOT have parent column"
    );
    assert!(
        !columns.contains("content"),
        "Blocks field should NOT have parent column"
    );
    assert!(
        !columns.contains("featured_posts"),
        "Has-many field should NOT have parent column"
    );
}
...
fn global_array_field_save_and_read() {
    ...
    let conn = pool.get().expect("DB connection");
    let doc = query::get_global(&conn, "homepage", &def, None).expect("Get global failed");
...
fn global_has_many_field_save_and_read() {
    ...
    let posts = doc
        .get("featured_posts")
        .expect("featured_posts should be populated");
...
fn global_group_field_preserved() {
    ...
    assert!(
        seo.is_object(),
        "seo should be an object (reconstructed from sub-columns)"
    );
...
fn global_empty_join_data_returns_empty_arrays() {
    ...
    let posts = doc
        .get("featured_posts")
        .expect("featured_posts should exist");
...
fn global_alter_table_adds_new_columns() {
    ...
    assert!(
        doc.fields.contains_key("description"),
...
fn hydrate_document_reconstructs_group_for_collections() {
    ...
    assert!(
        !doc.fields.contains_key("seo__meta_title"),
        "Sub-column should be removed"
    );
    let seo = doc.get("seo").expect("seo should exist");
...
fn collection_alter_adds_group_sub_columns() {
    ...
    assert!(
        columns.contains("seo__meta_title"),
        "Should have seo__meta_title sub-column"
    );
    assert!(
        columns.contains("seo__meta_description"),
        "Should have seo__meta_description sub-column"
    );
    assert!(
        !columns.contains("seo"),
        "Should NOT have single seo column"
    );

    // Old data preserved, new sub-columns are NULL
    let old_doc = query::find_by_id(&conn, "articles", &def, &doc.id, None)
        .unwrap()
        .unwrap();
    ...
    let seo = updated
        .fields
        .get("seo")
        .expect("seo should exist after hydration");
    assert_eq!(
...
fn global_alter_adds_group_sub_columns() {
    ...
    assert!(
        columns.contains("seo__og_image"),
        "Should have seo__og_image sub-column"
    );
    assert!(
        !columns.contains("seo"),
        "Should NOT have single seo column"
...
fn collection_alter_adds_localized_group_columns() {
    ...
    assert!(
        ...
        "Non-localized sub-column should exist"
    );
    assert!(
        !columns_v1.contains("seo__meta_title__en"),
        "Locale columns should not exist yet"
    );

    // Second sync: add a new localized sub-field to the group
    def.fields[1].fields.push(
        FieldDefinition::builder("og_description", FieldType::Text)
            .localized(true)
            .build(),
    );
    {
    ...
    assert!(
        columns_v2.contains("seo__meta_title"),
        "Original sub-column preserved"
    );
    assert!(
        columns_v2.contains("seo__og_description__en"),
        "EN locale column added"
    );
    assert!(
        columns_v2.contains("seo__og_description__de"),
        "DE locale column added"
    );
    assert!(
        !columns_v2.contains("seo__og_description"),
        "Non-localized column should NOT exist"
    );
}
